...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        If the "featureVals" smaller than 0, it means the classifier thinks that the image is a face.
        Therefore, "ans"=1, and vice versa.
        weight * (ans-label) = error.
        Add all the images' error to sum, and find the smallest. -> weak classifier
        Finally, turn the HaarFeature class that we chose to WeakClassifier Class. 
        ...
        for i in range(len(featureVals)):
          sum=0
          for j in range(len(featureVals[0])):
            if featureVals[i][j]<0:
              ans=1
            else:
              ans=0
            sum=sum+weights[j]*abs(ans-labels[j])
          if min>sum:
            min=sum
            minindex=i
        weakclassifier_class = classifier.WeakClassifier(features[minindex])
        bestClf = weakclassifier_class
        bestError=min
        print(min)
        """
        #for i in range(len(featureVals)):
        ...
        for j in range(len(featureVals)):
          ...
          for i in range(len(iis)):
            if featureVals[j][i]<0:
              ...
              ans=0
            error += (weights[i]*abs(ans-labels[i]))
            # error = error/len(iis)
          if error < bestError:
            bestError = error
            bestIndex = j
        clf = WeakClassifier(features[bestIndex])
        bestClf = clf
