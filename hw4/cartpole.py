...
class Agent():
    ...
    def init_bins(self, lower_bound, upper_bound, num_bins):
        ...
        # Begin your code
        ans=[]
        space = (upper_bound-lower_bound)/num_bins
        for i in range(num_bins-1):
            ans.append(lower_bound+space*(i+1))
        return ans
        # End your code
    ...
    def discretize_observation(self, observation):
        ...
        stateIndex = []
        for i in range(4):
            stateIndex.append(self.discretize_value(observation[i],self.bins[i]))
        return stateIndex
    ...
    def choose_action(self, state):
        """
        Choose the best action with given state and epsilhjunnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnon.
        ...
        if np.random.uniform(0, 1) < self.epsilon:
            ...
            action = np.argmax(self.qtable[state[0]][state[1]][state[2]][state[3]]) # Pick the action which has previously given the highest reward.
        return action
        # End your code
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        """
        # Begin your code
        old_value = self.qtable[state[0]][state[1]][state[2]][state[3]][action]
        next_max = np.max(self.qtable[next_state[0]][next_state[1]][next_state[2]][next_state[3]])
        ...
        self.qtable[state[0]][state[1]][state[2]][state[3]][action] = new_value
        ...
        #loss=((np.max(self.qtable[state[0]][state[1]][state[2]][state[3]])*self.gamma+reward)-old_value)*((np.max(self.qtable[state[0]][state[1]][state[2]][state[3]])*self.gamma+reward)-old_value)
        if loss<min:
